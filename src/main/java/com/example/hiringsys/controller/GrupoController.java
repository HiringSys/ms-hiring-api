...
public class GrupoController {
    ...
    @GetMapping public List<GrupoResponse> listar(@RequestParam(required=false) String nome,@RequestParam(required=false) EstadoGrupo estado){List<Grupo> grupos=nome!=null?service.buscarPorNome(nome):estado!=null?service.buscarPorEstado(estado):service.listarTodos();Map<Long,Long> contagens=service.contarParticipantes(grupos);return grupos.stream().map(grupo -> grupoResponse(grupo,contagens.getOrDefault(grupo.getId(),0L))).toList();}
    @GetMapping("/{id}") public GrupoResponse buscar(@PathVariable Long id){Grupo grupo=service.buscarPorId(id);return grupoResponse(grupo,service.contarParticipantes(id));}
    @PostMapping public ResponseEntity<GrupoResponse> criar(@Valid @RequestBody GrupoRequest request){Grupo salvo=service.salvar(toEntity(request));return ResponseEntity.created(URI.create("/grupos/"+salvo.getId())).body(grupoResponse(salvo,0));}
    @PutMapping("/{id}") public GrupoResponse atualizar(@PathVariable Long id,@Valid @RequestBody GrupoRequest request){Grupo atualizado=service.atualizar(id,toEntity(request));return grupoResponse(atualizado,service.contarParticipantes(id));}
    ...
    private Grupo toEntity(GrupoRequest r){Grupo g=new Grupo();g.setNome(r.nome());g.setArea(r.area());g.setEstado(r.estado());g.setDisponiveis(r.disponiveis());g.setCargo(r.cargo());g.setLimiteAprovados(r.limiteAprovados());g.setEmailEquipe(r.emailEquipe());return g;}
    private GrupoResponse grupoResponse(Grupo g,long quantidadeParticipantes){return new GrupoResponse(g.getId(),g.getNome(),g.getArea(),g.getEstado(),g.getDisponiveis(),g.getCargo(),g.getLimiteAprovados(),g.getEmailEquipe(),quantidadeParticipantes,g.getCriadoEm());}
